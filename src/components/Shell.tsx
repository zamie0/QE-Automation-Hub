...
export function Shell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex">
      <aside className="hidden lg:flex flex-col w-64 m-4 mr-0 rounded-3xl glass p-5 sticky top-4 self-start h-[calc(100vh-2rem)] overflow-visible z-40">
        <Link to="/" className="flex items-center gap-3 px-2 mb-8">
        <div className="h-10 w-10 rounded-2xl shadow-lg overflow-hidden">
          <img
            src="/QE-logo.png"
            alt="icon"
            className="h-full w-full mt-0.5"
          />
        </div>
          <div>
...
function ProfileMenu() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl glass-strong text-sm text-foreground/70 hover:bg-white/80 hover:text-foreground transition"
      >
        <div className="relative h-8 w-8 rounded-lg bg-[image:var(--gradient-primary)] grid place-items-center text-white text-xs font-semibold shadow">
          HZ
          {unread > 0 && (
        ...
        <div className="min-w-0 flex-1 text-left leading-tight">
          <div className="text-sm font-medium truncate">Hazami</div>
          <div className="text-[10px] text-muted-foreground truncate">System Developer</div>
        </div>
        <ChevronDown className="h-4 w-4 text-muted-foreground" />
      </button>

      {open && (
        <div className="absolute left-full bottom-0 ml-3 w-72 rounded-2xl bg-white p-2 shadow-xl z-[5000] max-h-[calc(100vh-2rem)] overflow-y-auto">
